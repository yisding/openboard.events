...
> **Current rebaseline (rev. 11, Mon Aug 10):** **PR #94 merged** (status §2f) — the P3→P5 roadmap run landed P3 compliance hardening (CSP/HSTS, a CSRF chokepoint in `defineHandler`, DB-backed rate limits), email compliance (a signature-verified Resend bounce/complaint webhook, List-Unsubscribe, suppression enforcement), an R2 orphan sweep and backup/rollback runbooks, the forms debt clear-out (M12 generalized past `context='cfp'`, M13b's rules UI mounted live, M14's `upsertDraft` gap closed, M24 built on the generalized engine), a triaged/fixed e2e and app-bug backlog, and the P5 product-completeness modules M50 (partial), M51, M52 (partial), M53, and M54, landing five additive migrations (`drizzle/0004`–`0008`) applied to `sb-dev`/`sb-test`. M42–M49 remain blocked pending explicit owner re-authorization of the M42 (Better Auth) hold; M55 stays blocked on tenancy (M43/M44). No module is `DONE` under the evidence rules — what remains is the deployed verification queue (redeploy, smoke, a full Playwright rerun, deployed 429/CSP/webhook checks, then the deployed demonstrations) and a small code queue (M50/M52's partial remainders, e2e leftovers, and the M42 chain once re-authorized) (status §6). Read [`status.md`](status.md) (§2c–§2f are the ledger); the product overlay is [`product-roadmap.md`](product-roadmap.md). The original wave table remains the dependency choreography.

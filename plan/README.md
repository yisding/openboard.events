...
> **Current rebaseline (rev. 9, Sun Aug 9 night):** **All agents are shut down and every module claim is released** — Status cells in `modules/*.md` describe evidence on `main`, not assignments; the claiming protocol below resumes when agents are turned back on. `main` is at PR #89: since rev. 8 the form builder went server-backed (#82), the full evaluation stack (#78–#85), the portal task runtime (#88), and the conflict engine (#89) merged, the submissions/evaluation seeds landed (#67/#83), M17 gained its drawer (#79/#84), and the delta #17 DMARC header gate passed (`DECISIONS.md`). The preview has since been **redeployed from `main`** (`673eac6`, version `1da1951d`) with all 7 post-deploy smoke checks green and zero skips, so the merged-but-unproven modules are live and their deployed AC can be attempted (status §6). Read [`status.md`](status.md) before claiming work (§2c–§2d are the per-module PR ledger); the product overlay beyond the judged bar is [`product-roadmap.md`](product-roadmap.md). The original wave table remains the dependency choreography.

...
11. **Rich-text XSS**: Welcome content and organizer HTML render on a public page — store sanitized HTML from the shared allowlist and render only through `<RichTextView>`, the one audited `dangerouslySetInnerHTML` site. Portal resource pages use the separately constrained wide profile from the same sanitizer.

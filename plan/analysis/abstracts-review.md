...
10. **AI-assisted review**: implement, if at all, as one authenticated "Generate AI review" route writing a Review row from a synthetic "AI Reviewer" with the model's rationale as the comment — no pipelines, no auto-runs. The route authorizes against the selected event/abstract (admin/reviewer scope, event-scoped like every mutation) and is idempotent: the write upserts on the (evaluation plan, submission, AI-reviewer) uniqueness, so a retry updates the existing AI review instead of inserting a duplicate row and inflating the aggregate rating.

...
16. [CORE] **Send pipeline**: outbound email uses the one Resend adapter; every attempt is a `communication_logs` outbox/audit row (recipient, template, queued/sent/failed/skipped status, provider id, timestamp) with a key from the seven shared natural-key builders so retries and double-fired triggers cannot double-send.

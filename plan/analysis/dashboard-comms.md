...
16. [CORE] **Send pipeline**: outbound email via Resend (swyx explicitly blessed it) or Cloudflare Email; every send recorded in a `communication_log` (recipient, template, status sent/failed, provider id, timestamp) with an idempotency key (`event_id + template_key + entity_id + revision`) so retries and double-fired triggers can't double-send.
...
18. [NICE] **Comms history per speaker**: timeline of what was sent to whom, surfaced on the speaker detail page from `communication_log`.
...
21. [NICE] **Airtable export**: include `speakers`, `submissions`, `tasks` (with completion status), and `communication_log` in the one-way Airtable sync so this area contributes to the bonus.
...
- **EmailTemplate** (owned) — id, event_id, key (`submission_received` | `accepted` | `declined` | `task_assigned` | `task_reminder` | `schedule_assigned` | `schedule_changed`), subject, body (markdown/HTML with `{{vars}}`), enabled (bool), updated_at.
- **ReminderRule** (owned) — id, event_id, template_key, offset (interval relative to task due date, e.g. `-P7D`, `-P1D`, `+P1D`), enabled.
- **CommunicationLog** (owned) — id, event_id, speaker_id, template_key, subject_rendered, status (`queued` | `sent` | `failed`), provider_message_id, idempotency_key (unique), ics_uid (nullable), sent_at.
...
5. **Fixed template set with editable subject/body**: seven predefined template keys; no arbitrary-template builder, no per-speaker overrides, no rich drag-drop email designer — a textarea with variable insert buttons and live preview is enough.
...
7. **Cron-scan scheduler** instead of per-reminder durable Workflows: one Cloudflare Cron Trigger every 15 min doing an idempotent scan is simpler, restart-safe, and still earns the Cloudflare-infra bonus.

...
This document is the short execution overlay for the current tree. It turns
...
The core server-backed paths are present across CFP intake, review, speaker
operations, portal tasks, agenda scheduling, public pages, embeds, and the
organization CRM. Upload handoff and CFP participant/draft implementation are
complete; the remaining work is concentrated in deployment proofs and clean
integration runs:

- Upload attachment returns the stored version and refreshes the task only
  after attachment succeeds; a fresh R2-backed run remains.
- The CFP wizard supports repeatable co-speakers, scoped answers, saved draft
  restoration, and role-aware review; close-date and deployed browser
  verification remain.
- Review, speaker-roster, and deliverables flows need a clean reset/reseed and
  redeployed run after their latest seed and UI changes.
- Assisted agenda placement has server-side stale-write coverage; the remaining
  work is a two-tab client rollback proof plus public-surface verification.
- The organization CRM has directory/UI/API coverage, and merge recovery now
  has an immutable snapshot, organization-scoped audit lookup, transactional
  restore, and integration coverage. A focused browser run remains.
...
1. Exercise two tabs against the same session: one stale `moveSession` write must
   return typed `STALE_WRITE`, roll back the losing client, and leave the winner
   intact.
...
2. Add a focused browser spec covering the highest-risk transitions: import
   retry, merge preview/commit/recovery, pipeline movement, and event push.
3. Use the organization-scoped merge audit lookup to inspect every merge and
   verify the compare-and-restore recovery path against a clean fixture.

Done when the CRM UI and API agree after reload, every mutation is organization
scoped, and a merge can be investigated and safely restored from its audit record.
...
Keep cross-feature changes on the existing single-writer paths and reuse the
established outbox, publication-query, task-assignment, and CAS boundaries.
...
- [ ] Agenda conflict, assisted placement, and stale-write client checks pass.
...
- [x] CRM merge audit lookup and recovery implementation have focused integration coverage; browser pass remains.

...
| | |
|---|---|
| **Status** | IN PROGRESS — **IMPLEMENTED on branch (rev. 10 run)**, no active claim. On `main` plus the uncommitted M17+M18 pass: the prior database-backed table/tabs/decision-bar/drawer, and now `updateSubmissionFields` (a guarded UPDATE on `expectedRowVersion` with tag reconciliation riding the same statement, placed in `mutations.ts` per M18's single-writer guardrail), its PATCH route, the manual "Add Abstract" flow (`AddAbstractDrawer` + shared `AbstractFields` editor, also used by a now-editable detail drawer with 409 STALE_WRITE messaging), and `getSubmissionVocabulary` feeding both editors. Remaining before `DONE`: the deployed triage AC (curl of notify twice, deployed lifecycle checks, Playwright abstracts-decide). See [`../status.md`](../status.md). |

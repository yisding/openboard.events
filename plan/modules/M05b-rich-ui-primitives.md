...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED**, no active claim. All six primitives, the rich kitchen sink and the client-bundle gate landed in #27/#28, and the review recovery merged in #60 (presign/finalize network failures surface as `<FileUpload>`'s retryable error state; the configured rich-text placeholder renders). Remaining for `DONE`: `<FileUpload>`'s AC needs a **browser** upload against the deployed preview, which is also M07's outstanding CORS proof. |

...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED** ([PR #12](https://github.com/yisding/symmetrical-happiness/pull/12)): single-use hashed magic-link/OTP tokens with attempt limits, AES-GCM-encrypted `portal_login` delivery payloads, durable portal sessions, impersonation, cookie middleware, and issuance throttling serialized under a contact row lock. The contingency-grant `features/portal/server/contacts.ts` ownership returned to [M21](./M21-portal-shell.md) at merge (resolution #13 still forbids any other `contacts` write path). **A deployed OTP session and a delivered `portal_login` email were proven at status rev. 7**, and the portal shell binds to the authenticated contact since #29–#33. Remaining before `DONE`: AC sign-off, including the phone-width AC. See [`../status.md`](../status.md) §2a. |

...
| | |
|---|---|
| **Status** | IN PROGRESS — reclaimed by Codex for the PR #62 follow-up: reject a known field's wrong answer discriminator before empty-value handling, including optional fields and draft saves. Earlier review recovery already validates authored field types (including email/URL shape) and treats `mapsTo` drift as structurally incompatible. **The pipeline and both routes are merged** (#35, #36): `runSubmitPipeline`, `deriveMappedFields`, `isStructurallyCompatible`, `POST /api/internal/forms/[formId]/submit` and `/draft`, with the public wizard calling the server path — and **a deployed submit was proven at status rev. 7** (a real submission through the deployed route into Neon with routing applied). Remaining: this focused discriminator-ordering fix and AC sign-off. |

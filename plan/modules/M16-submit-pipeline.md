...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED**, no active claim. The pipeline and both routes (#35/#36): `runSubmitPipeline`, `deriveMappedFields`, `isStructurallyCompatible`, `POST /api/internal/forms/[formId]/submit` and `/draft`, with the public wizard calling the server path — and **a deployed submit was proven at status rev. 7**. Every review-recovery tranche has since merged: field-type/answer-discriminator validation and `mapsTo`-drift handling, the batched answer writes (#73), the empty-value type-check ordering (#74), and the route-contract follow-ups (#87: event ownership derived from the route form, draft metadata returned, participant client IDs/emails resolved server-side). Remaining: AC sign-off. |

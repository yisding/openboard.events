...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED** ([PR #88](https://github.com/yisding/symmetrical-happiness/pull/88), with review follow-ups `a1f577c`/`24c1949`): the whole runtime. Steps 2, 4–10 complete — `listMyTasks`/`getMyTask` over `task_assignments_v`, the three completion paths with their two audited `withTx` bodies, field-scoped write-back through `updateContactFields` and an allow-listed submission UPDATE, the three portal routes, the speaker list and detail pages (manual, file and form modes through the real `<FormFieldRenderer>` at `mode="edit"`), and step 11's viewers exported from `@/features/portal/index.client`. 18+ PGlite cases. Step 3 ships grouped sections rather than an Accordion (no such primitive exists; `shared/ui` is M05b's). **Blocking `DONE`:** the phone-width deployed run-through and the dashboard count dropping on the next poll (the running preview predates this merge). See [`../status.md`](../status.md). |

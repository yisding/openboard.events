...
| | |
|---|---|
| **Status** | IN REVIEW — **PR-OPEN** in [PR #88](https://github.com/yisding/symmetrical-happiness/pull/88) by Claude: the whole runtime. Steps 2, 4–10 complete — `listMyTasks`/`getMyTask` over `task_assignments_v`, the three completion paths with their two audited `withTx` bodies, field-scoped write-back through `updateContactFields` and an allow-listed submission UPDATE, the three portal routes, the speaker list and detail pages (manual, file and form modes through the real `<FormFieldRenderer>` at `mode="edit"`), and step 11's viewers exported from a new `@/features/portal/index.client`. 18 PGlite cases. Step 3 ships grouped sections rather than an Accordion (no such primitive exists; `shared/ui` is M05b's). A pre-PR subagent review caught four real defects, all fixed on the branch. **Blocking `DONE`:** merge, then the phone-width deployed run-through and the dashboard count dropping on the next poll. See [`../status.md`](../status.md). |
...
- [./M24-portal-form-builder.md](./M24-portal-form-builder.md) — **soft.** The form-mode slice renders the **2 seeded portal forms** (also from `scripts/seed/portal.ts`, authored by M24 Step 1 which ships them *before* any builder UI). M24's builder UI is never on this module's critical path — that is exactly why cut-line #13 is free.

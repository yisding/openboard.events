...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED-PARTIAL**, no active claim. The preview Worker is live at `https://sb-web-preview.yi-ding.workers.dev` with a real Neon `/api/health` round-trip and the artifact inside the Workers Free budget (1679 KiB gzip at status rev. 7). Resend delivery, revalidate-60 edge caching, and `frame-ancestors` were proven at status rev. 7; the CI-performance tranche landed on `main` (`a0c6265` — independent gates parallelized without weakening coverage). The DMARC header gate is green (`dmarc=pass` with aligned identities on a Gmail delivery, recorded in `DECISIONS.md` Sun Aug 9). Remaining: browser R2 presign/CORS, a deployed auth-throttle proof, and a green `Deploy` workflow run from `main`. See [`../status.md`](../status.md). |

...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED-PARTIAL** steps 1–4 from PR #1: guarded no-op routes, job contracts, dispatcher worker, production env, and deploy command. Remaining: deployed worker→web proof, secrets on both workers, tail evidence, and AC-gated stub swaps. See [`../status.md`](../status.md). |

...
| | |
|---|---|
| **Status** | IN PROGRESS — **merged** ([PR #16](https://github.com/yisding/symmetrical-happiness/pull/16); a satisfied hard dependency for downstream claims): dispatcher, authoritative templates, renderer, context rebuilding, retry/claim pipeline, Resend adapter, job wiring, and seed. **Gmail delivery through a deployed dispatch proven at status rev. 7**, and the delta #17 header gate is green — `dmarc=pass` with aligned identities on a Gmail delivery (recorded in `DECISIONS.md`, Sun Aug 9). Remaining before `DONE`: the Outlook probe, calendar-invite delivery, and a production sending key. See [`../status.md`](../status.md). |

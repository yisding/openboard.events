...
- **Hard (blocks start):**
  ...
  - [M04](./M04-shared-libs.md) — `enqueueEmail(tx, {templateKey, contactId, idempotencyKey, refs})`, `getEnv()` (`EMAIL_MODE`, `EMAIL_ALLOWLIST`, `EMAIL_FROM`, `RESEND_API_KEY`, `APP_BASE_URL`), `sanitize()`, `time.ts`'s `formatInZone`, `errors.ts`.
...
- **Soft (start against stub/fixture):**
  - `issuePortalToken(dbOrTx, {contactId, eventId, purpose, ttl}): Promise<{raw, expiresAt}>` ([M06b](./M06b-portal-auth.md), lands Sat PM) — a Phase-0 throwing stub exists in contracts. Note the **object** return (destructure `raw`) and the `DbOrTx` first parameter. Code against the exact signature; until it returns real tokens, have the context builder fall back to `${APP_BASE_URL}/portal/${slug}/login?email=…` and log a `TODO(M06b)` line. **Swap step:** import from `@/features/auth` and delete the fallback branch (one function, `buildPortalLink`).
...
3. **The send-time context builder** — `server/context.ts`. The log row carries **ids, not truth** (`contact_id`, `submission_id`, `session_id`, `task_id`, `template_key`, `ics_uid`); this is where truth is re-read.
   ...
   - **Token minting happens HERE, at send time** (binding resolution #12). `issuePortalToken` returns an **object** — interpolating it directly would put `[object Object]` in every judged email — so destructure it:
     ...
     ```
     The first parameter is typed **`DbOrTx`** ([M02](./M02-shared-contracts.md) §11 / [M06b](./M06b-portal-auth.md)), which is what makes passing the neon-http `db` handle legal: the helper performs a single INSERT, so this must **not** open a 5th `withTx` path (binding resolution #4). The **30 d** TTL is the documented dispatcher-minted row in M06b's TTL table (a link inside an inbox must outlive the 15/60-minute interactive TTLs).
...
5. **Per-row pipeline.** For each claimed row: `buildContext` → (skip path: `UPDATE … SET status='skipped', error=$reason` and continue) → load the event's `email_templates` row for the key; **`enabled = false` → `status='skipped'`, reason `template disabled`** → **per-form override:** for `submission_received`, if the submission's `forms` row has a non-null `confirmation_subject` / `confirmation_body_html`, render **those** instead of the event template (same variable contract, same escaping, same validation) — this is what makes [M14](./M14-form-settings-notifications.md)'s "Customize" disclosure a real feature rather than a dead column → `renderTemplate` → persist `subject_rendered` + `body_rendered_html` **before** the provider call (the judge-mode fallback surface must exist even if Resend errors) → `EMAIL_MODE` gate → send → mark terminal.
   - **EMAIL_MODE matrix** (PLAN §2, authoritative — do not invent a third mode):
     ...
     - `EMAIL_FALLBACK_UI=1` does **not** change the dispatcher — it is [M06b](./M06b-portal-auth.md)'s verify-page flag. The dispatcher's contribution to that fallback is that `body_rendered_html` always holds a working magic link, surfaced by [M37](./M37-comms-admin-ui.md)'s log detail and documented in `docs/demo-script.md`.
...
- **`withTx` stays at 4 audited functions** (resolution #4) — the dispatcher is single-statement claim + single-statement marks on neon-http. Do not add a transaction here.
...
- Blocked on `issuePortalToken`: use the documented fallback link and keep going — the swap is one function.

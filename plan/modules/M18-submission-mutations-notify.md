...
| | |
|---|---|
| **Status** | IN PROGRESS — **the Saturday slice is merged** (#34): `nextSubmissionCode`, `createSubmission`, `upsertDraft`, `formatCode`, `assertTransition` and `toPortalStatus`, with eleven PGlite cases. This is what the Sat-night thin slice needs from WS-C. **The Sunday half landed in #57**: `transitionStatus`, `notifyQueues`, and the transition/notify route handlers, with 11 further PGlite cases. Remaining: `updateSubmissionFromCfp`, `withdraw`, `getAcceptedForScheduling`, the withdraw route — and a UI consumer (M17's drawer/bulk actions). |
...
- [M04](./M04-shared-libs.md) — `withTx` (confined Pool), `enqueueEmail(tx, {eventId, templateKey, contactId, idempotencyKey, refs})`, `defineHandler`, `errors.ts`, `time.ts`, `sanitize()`.
...
3. **`createSubmission` — the audited `withTx` path.** Order inside one transaction (this order is load-bearing):
   ...
   3. If `enforce.limit !== false`: `count(*) FROM submissions WHERE event_id=$1 AND form_id=$2 AND submitter_contact_id=$3 AND status NOT IN ('draft','withdrawn')` vs `COALESCE(form.submission_limit, events.submission_cap_per_user)` → `AppError('LIMIT_REACHED')`. **Drafts never consume the limit** — this matches data-model §3.5 and PLAN resolution #3.
...
6. **`notifyQueues` / `notifyDecisions` in `withTx`.** Two guarded bulk flips, then the outbox:
   ...
   - `enqueueEmail(tx, { eventId, templateKey: 'submission_accepted' | 'submission_declined', contactId, idempotencyKey: idem.decision(eventId, submissionId, notify_revision), refs:{ submissionId } })`.
...
- **Resolution #4 (drivers).** `withTx` (WebSocket Pool) is allowed in exactly eight runtime repository functions; four of them are yours: `createSubmission`, `upsertDraft`, `updateSubmissionFromCfp`, and the internal `notifyDecisions` body used by `notifyQueues`. Everything else in this module uses `neon-http`.

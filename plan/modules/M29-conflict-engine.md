...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED** ([PR #89](https://github.com/yisding/symmetrical-happiness/pull/89)), and its server caller is satisfied. `conflicts.ts` speaks the frozen contract — `ScheduledSession` in epoch milliseconds, `toScheduledSession` returning `null` for NULL-time rows, and `detectConflicts` returning `ConflictDTO[]` from a per-subject sweep with deterministic ordering. The suite now includes a deterministic 200-seed randomized harness with an independent pairwise oracle, permutation invariance, half-open interval checks, self-pair/duplicate guards, and a generated disjoint schedule. **Next:** run the full repository/e2e gates and verify the agenda schedule flow in a fresh preview deployment. See [`../status.md`](../status.md). |

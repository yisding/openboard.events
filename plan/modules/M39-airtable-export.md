...
## Dependencies
- **Hard (blocks start):**
  - [M03](./M03-db-schema-migrations.md) — `airtable_sync_state` (`UNIQUE(event_id, table_name, record_pk)` — event-scoped, per data-model §3.11), `airtable_sync_runs`, and **every exported view carrying `greatest(a.updated_at, b.updated_at, …) AS updated_at`** (the ★ delta in PLAN §3 — without it, a speaker bio edit that only touches a joined table is silently skipped). Verify the column exists on `accepted_speakers_v`-backed sources before writing sync code.
...
```
- `POST /api/internal/airtable/[eventId]/sync` — admin-only manual trigger: auth-check, then `ctx.waitUntil(runAirtableSync(300))` and return 202. **It does not insert the run row** — `runAirtableSync` owns the run-row insert for both the manual and cron paths (step 6's single-flight guard), so the manual path can neither see its own row as an "existing run" nor create a duplicate record.
...
1. **★ SATURDAY, SCHEDULED — provisioning + the hand-run verification (30 min, no app code).**
   ...
   - Store `AIRTABLE_API_KEY` (PAT, scope `data.records:write` + `schema.bases:read`) and `AIRTABLE_BASE_ID` on the **web worker only**: `.dev.vars` for local, and `wrangler secret put … --env preview` / `--env production` against **`sb-web-preview` and `sb-web`** — never on the jobs workers (`sb-jobs*` receives only `APP_BASE_URL` + `CRON_SECRET`, per [`../environments.md`](../environments.md); the hard-dependency note above is binding). Write the base/table ids into `DECISIONS.md`.
...
5. **Sources + watermark** — `server/sources.ts`. One function per table returning `{ pk, updatedAt, fields }[]`, each reading **views only** (status filtering comes free: `Speakers` from `contacts ∩ accepted_speakers_v` + `speaker_outstanding_v.open_count`; `Sessions` from `published_sessions_v` + speakers; `Task Status` from `task_assignments_v`; `Submissions` from `submissions` non-draft + `submission_ratings_v`; `Comms Log` from `communication_logs`).
   - **Watermark = a composite `(updated_at, record_pk)` cursor, never a bare timestamp.** Query shape: `… WHERE (v.updated_at, v.pk) > ($wTs, $wPk) ORDER BY v.updated_at ASC, v.pk ASC LIMIT $batch` — PostgreSQL row-value comparison gives the strict lexicographic boundary. A timestamp-only `updated_at > $w` cursor silently drops rows: seeded data shares timestamps, so a batch boundary landing mid-timestamp would permanently skip the remaining rows at that instant, and `>=` would re-send them forever. `v.updated_at` is the view's `greatest(...)` aggregate — the ★ requirement: a speaker bio edit changes `contacts.updated_at`, and the Sessions row that joins that speaker must therefore also move — the aggregate makes that true without per-table hacks.
   - **Watermark storage** (PROPOSED, zero schema change): a sentinel row per (event, table) in `airtable_sync_state` — `record_pk = '__watermark__'`, `airtable_record_id = '-'`, `content_hash = '<ISO timestamp>|<last record_pk>'` (both cursor halves) — upserted **after every successfully processed source batch, including a batch where every row was hash-skipped** (the cursor must advance past skipped rows, or the same batch re-reads on every run and the sync never progresses), so a crashed run resumes exactly where it stopped. Mirror the same values into the finishing run's `airtable_sync_runs.stats.watermarks` for the admin UI.
...
6. **The sync loop** — `server/sync.ts`. `runAirtableSync(budget = 300)`:
   ...
   2. For each table in a fixed order, until the 300-record budget is spent: read the next batch (100 rows) past the composite watermark cursor → build fields → `content_hash = sha256(canonical JSON of fields)` (Web Crypto, stable key order) → diff against `airtable_sync_state`: unknown pk → send; known + hash differs → send; hash equal → **skip** (counted, logged) → `upsertBatch` in chunks of 10 → upsert `airtable_sync_state` rows with the returned `airtable_record_id` + new hash → advance the watermark sentinel to the batch's last `(updated_at, record_pk)` — **also when the batch sent zero records because every row hash-skipped** (step 5's rule).

...
- `wide` profile: default **plus `iframe[src ^https?:]`** with `allowfullscreen`, `width`, `height` — used **only** by `resource_pages.body_html` ([M26](./M26-resource-pages.md)), the brief's "HTML embed support".
...
- Tests: `<script>alert(1)</script>` → empty; `<img src=x onerror=alert(1)>` → attribute stripped; `<a href="javascript:alert(1)">` → href dropped; `<iframe>` stripped under `default`, kept under `wide`; sanitize is idempotent (`sanitize(sanitize(x)) === sanitize(x)`).
- **Done when:** those five assertions pass and the S3 verdict's implementation is the one shipped.

...
1. **Contract-first slice: the pure builder + its golden fixtures.** Write `ics.ts` with the signatures above and `ics.test.ts` asserting against committed `__fixtures__/request.ics`, `cancel.ics`, `feed.ics`. This slice needs **only** [M02](./M02-shared-contracts.md) — start it any time, even before [M34](./M34-comms-outbox-dispatcher.md) finishes.
   ...
   - **REQUEST/CANCEL require `attendee`, and its `email` must be the message's `To:` recipient** — `buildInvite` throws `AppError('VALIDATION', …)` on a REQUEST/CANCEL without one (feeds are the only attendee-less shape). Gmail and Outlook silently degrade an invite whose ATTENDEE does not match the recipient to a dead `.ics` attachment — no chip, no RSVP (rev. 3 delta #16).
...
2. **★ SATURDAY, SCHEDULED — the canned real-inbox render check.** No app code. Hand-write (or generate with step 1's builder from a node one-liner) a `METHOD:REQUEST` invite for a fake session, base64 it, and `curl` it through Resend as an attachment with `content_type: 'text/calendar; charset=utf-8; method=REQUEST'` to **one real Gmail address and one real Outlook.com address** (team-owned inboxes; the resend.dev shared domain + allowlist suffices here because the recipient is us).
   ...
   ```
   If a team member has an M365/corporate-Exchange mailbox, CC it — corporate Outlook parses invites differently from Outlook.com; a pass there is free signal, a fail is not a gate.
...
- **`ORGANIZER` must be a real mailbox on the verified Resend domain** or Gmail suppresses the RSVP buttons — **and it is immutable per UID** (delta #16): derived from `EMAIL_FROM` alone, and `EMAIL_FROM` freezes the moment the first invite is sent. Change it and every outstanding invite orphans — reschedules duplicate instead of updating, CANCELs are ignored.

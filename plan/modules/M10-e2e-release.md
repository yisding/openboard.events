...
| | |
|---|---|
| **Status** | IN PROGRESS — **IMPLEMENTED on branch (rev. 10 run)**, no active claim. Every placeholder step body in the six specs (`cfp-submit`, `abstracts-decide`, `admin-setup`, `agenda-schedule`, `portal-tasks`, `public-embeds`) now has a real implementation — 24 tests across 6 files — and all 17 `e2e/helpers/landed.ts` gates are flipped to `true` in the same change. New helpers (`e2e/helpers/{ids,db,seeded,auth}.ts`) resolve seeded ids deterministically and query Postgres directly for the two assertions the UI can't make. Two read-only specs (`cfp-submit`'s closed-form and 390px cases) passed against the live preview; the full suite needs `NEON_TEST_URL` and a redeploy of the working tree to run end to end. Remaining before `DONE`: a database-backed and fully deployed-preview run of all 24 tests, plus 7 bugs found and reported (not fixed, feature source off-limits) including a non-deterministic seed fan-out and an empty public speaker gallery on fresh seed. See [`../status.md`](../status.md). |

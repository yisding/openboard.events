...
| | |
|---|---|
| **Status** | IN PROGRESS — **step 1 (the CP1 six-spec skeleton) claimed** by the agent that holds M07, which is `IN REVIEW`; it is R1 step 6 and depends on nothing merged. Steps 2–12 remain unclaimed. **MERGED-PARTIAL** release docs, validation CI, smoke, and license scaffolding exist, and the smoke script passed against the deployed preview. Absent: the load test, production deployment, spend proof, and the release AC. See [`../status.md`](../status.md). |

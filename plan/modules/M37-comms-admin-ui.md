...
| | |
|---|---|
| **Status** | IN PROGRESS — **IMPLEMENTED on branch (rev. 10 run)**, no active claim. The admin UI now runs over the real merged server: `server/admin-mutations.ts` adds `listTemplates`/`saveTemplate` (409 STALE_WRITE), `listReminderRules`/`saveReminderRules` (replace-the-set), `getLogDetail`, and `listOpenAssignmentsForContact`. A Templates tab (8-key rail, cursor-position `{{var}}` chips, live preview), a Reminders tab, and a `<CommsLogTable>` (status/template/recipient filters, detail drawer) replace the demo store, plus a per-speaker "Send reminder now" wired to `sendReminderNow`/`nudgeOutbox`. The body editor deliberately stays a plain `<textarea>` (TipTap exposes no cursor API), per the module's own sanctioned fallback. 12 new PGlite tests cover token rejection, sanitization, and stale-write. Remaining before `DONE`: deployed/browser AC. See [`../status.md`](../status.md). |

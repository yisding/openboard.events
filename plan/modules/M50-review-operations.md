...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED (rev. 11 / PR #94), finish pass landed at rev. 12 / PR #95**, no active claim. Implemented on M19's merged evaluation stack (no second score store): round windows, `anonymize_authors`, typed criteria (numeric/select/text), `review_assignments` (explicit assignment authority + recusal), and reviewer provisioning/reminders land via additive migration `drizzle/0004_review_operations.sql`; blindness is built into the reviewer DTO (`identity` fail-closed default via `form_fields.review_visibility`), and the typed scorecard's arithmetic is shared by server/client/tests. UI: round windows + blind toggle + typed criteria in the plan editor, an assignment drawer, an invite dialog, and a queue rendering all three criterion kinds. The rev. 12 finish pass fixed the real seed/spec gaps this AC actually stood on (no server change was needed): `scripts/seed/forms.ts` never wrote `review_visibility` at all, so it now seeds and answers the two AC-named questions (`approach`/content, `employer`/fail-closed default); Round 2 now seeds a third reviewer plus a completed and a recused row so all four assignment states are visible at once; `e2e/review-operations.spec.ts` gained the half-open-window and answer-level-blindness assertions it lacked. Remaining before `DONE` is deployed evidence only: `landed.ts` keeps `M50: false` until the preview is redeployed (migrations through `0014` applied) and `sb-test` reseeded with the corrected fixture, then `e2e/review-operations.spec.ts` run against it; `reviewer_invited`/`review_reminder` Resend delivery is also still unverified. See [`../status.md`](../status.md) §2g. |

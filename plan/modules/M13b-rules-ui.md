...
| | |
|---|---|
| **Status** | IN PROGRESS — **IMPLEMENTED on branch (rev. 10 run), partial**, no active claim. `server/routing-mutations.ts` (list/save/delete/reorder over `routing_rules`, with a self-healing read sweep that soft-disables a rule whose field/option/tag reference was deleted), its three API routes, and all six owned components (`condition-row`, `rule-summary`, `visibility-rule-editor`, `builder-preview`, `routing-rules-panel`, its inline rule editor) are built to spec and pass PGlite tests. Remaining before `DONE`: M12's merged builder (PR #82) never created the stub mount points this work order assumed — it built its own inline visibility mini-editor and non-live preview directly in the M12-owned `form-builder.tsx` — so `RoutingRulesPanel`/`VisibilityRuleEditor`/`BuilderPreview` are not yet mounted in the live builder page; that swap (and extending `scripts/seed/forms.ts` with routing/visibility rules) needs an M12-owned change, plus deployed/browser AC. See [`../status.md`](../status.md). |

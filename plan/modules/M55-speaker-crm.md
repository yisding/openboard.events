...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED (rev. 12 / PR #95), partial — core and UI slices landed**, no active claim. Tenancy is unblocked; the CRM server/contracts layer uses additive migration `drizzle/0013_speaker_crm.sql` and the organization-scoped directory, history, import, merge, segments, pipeline, push, bulk-email, metrics, and UI routes are present. This slice adds migration `0017_crm_merge_recovery.sql`, an immutable compare-and-restore snapshot, an append-only recovery receipt, organization-scoped audit lookup, and a tested recovery route that restores surviving references without overwriting post-merge edits. **Next:** apply the new migration to the preview, run browser coverage for import retry, merge preview/commit/recovery, pipeline movement, event push, bulk email, and metrics, then record fresh deployment evidence. See [`../status.md`](../status.md) §2g. |

...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED** ([PR #82](https://github.com/yisding/symmetrical-happiness/pull/82), with review feedback addressed on the branch). The organizer form list and six-step builder use authenticated database queries and mutations; default creation, immutable snapshot versions, all eight field types, locked-field and post-submission structural guards, stale-write protection, counts, and reorder behavior are covered by PGlite acceptance tests. Remaining before `DONE`: deployed/browser authoring AC — the running preview predates this merge, so a redeploy comes first. Routing-rule authoring and deeper settings/notification behavior remain owned by M13b/M14. |

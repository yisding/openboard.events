...
| | |
|---|---|
| **Status** | IN PROGRESS — **claimed by Codex on branch `agent/m12-form-builder` for the database-backed builder recovery.** Seed reconciliation is merged (#46/#59/#64), as are the snapshot/public accessors (#36/#39). This claim owns the remaining builder surface: `listForms`, `getFormForBuilder`, structural/settings mutations, immutable snapshot publication, authenticated routes, and the six-step organizer UI. |

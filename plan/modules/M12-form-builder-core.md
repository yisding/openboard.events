...
| | |
|---|---|
| **Status** | IN PROGRESS — **Step 1's snapshot accessors are merged** (#36): `getPinnedSnapshot`, `getCurrentSnapshot`, `getActiveRoutingRules`, each event-scoped. Remaining: the builder itself — `listForms`, `getFormForBuilder`, `saveFormStep`, `compileAndPublish`, `getPublicForm`, and the six-step wizard UI. |

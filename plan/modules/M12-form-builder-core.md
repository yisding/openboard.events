...
| | |
|---|---|
| **Status** | IN PROGRESS — claimed by Codex for the PR #59 seed follow-up: reconcile organizer-created live fields that reuse a soft-deleted seeded key before reactivating the deterministic field. The earlier PR #46 recovery made re-seeds converge mutable authoring/routing values and mapped seeded Track/Format choices to their submission columns. **Step 1's snapshot accessors are merged** (#36): `getPinnedSnapshot`, `getCurrentSnapshot`, `getActiveRoutingRules`, each event-scoped. Remaining: the builder itself — `listForms`, `getFormForBuilder`, `saveFormStep`, `compileAndPublish`, `getPublicForm`, and the six-step wizard UI. |

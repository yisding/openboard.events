...
| | |
|---|---|
| **Status** | IN PROGRESS — **MERGED (rev. 11 / PR #94)**, no active claim. The rev. 10 blocker is closed: `builder-queries.ts`/`builder-mutations.ts` now accept `context`/`targetType` generically, and this run added the two entry points M12 never built — `deleteFormIn` (FK-RESTRICT precheck, M23-matching copy) and `duplicateFormIn` — wired into `/api/internal/forms/**` (`GET`/`POST` context/targetType, `DELETE`, new `POST .../duplicate`). The actual M24 surface is built: `src/features/portal/form-builder/components/{field-library.ts, portal-forms-page.tsx, portal-form-builder.tsx}` (portal forms list with duplicate/delete, single-page builder with an 8-triple standard-field library filtered by target type, and M14's `NotificationsStep` reused verbatim), mounted at `src/app/events/[eventId]/tasks/forms/**`; 11 integration tests passing (`tests/integration/portal-form-builder.test.ts`). **F0-forms-debt fix:** the field-CRUD save (`updateFieldIn`) previously allowed `maps_to` to be set to any value in the 17-entry allowlist regardless of the field's own form's `target_type` (a `target_type='submission'` portal form could accept `maps_to='contact.email'`), which would have made `deriveMappedFields`'s unconditional contact write-back silently overwrite a responding contact's real profile data. `guards.ts#assertMapsToMatchesTarget` now rejects a `maps_to` whose prefix doesn't match the form's `target_type` (CFP forms, `target_type === null`, are exempt); regression test added (`tests/integration/portal-form-builder.test.ts`, "rejects setting a maps_to whose target doesn't match..."). Remaining before `DONE`: deployed click-through (build/duplicate/delete on the preview), a sidebar/task-admin-page link to the new routes (currently reachable only by direct URL), bundle-size impact, and HTTP-layer proof of the `/duplicate`/`DELETE` routes. See [`../status.md`](../status.md). |

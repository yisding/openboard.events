...
- **Contracts-first**: `shared/contracts` compiles standalone (`scripts/check-contracts.ts`); DTOs are zod schemas with inferred types. **Contracts are hand-written zod v4, authoritative (PLAN resolution #5) — `drizzle-zod` is not a schema source for contracts.** The Phase-0 spike checks only whether the v4 ecosystem cooperates; its fallback is dropping to hand-written zod v3, never deriving contracts from the Drizzle schema. Fixtures in `shared/fixtures` are zod-parsed at test time so fixture drift fails CI.

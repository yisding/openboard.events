...
**CP1 (Sat noon):** schema migrated to all three DBs, seed loads, admin login works, every route renders a stub page, CI + deploy pipeline green, contracts FROZEN, **Resend domain verification checked (= the Authentication-Results pass probe, §2 / delta #17) — hard gate item: not propagated → debug/resubmit immediately, re-check Sat night, fallback decision Sun noon**. (CP1 gates M02/M03/M04/M05a/M06a only.)
...
AM: full bug bash of judge flows on production (all agents run the demo script cold; fix P0s only) — **including a complete OTP round-trip to one fresh Gmail AND one fresh Outlook address typed into the CFP wizard exactly as a judge would**, a portal magic-link + decision-email check on the same inboxes, and a calendar invite landed on the fresh Gmail's calendar from scheduling that speaker's session (delta #17). Midday: final seed reset, `wrangler rollback` rehearsed, post-deploy smoke green, comms log clean, **reimbursement proof compiled into `docs/spend/` + noted in the submission form**. PM: repo public (license, README, setup), submission form filled, walkthrough recording (optional), **submit by 8 PM PT** — a deliberate 2-hour buffer against upload/form failures. 8–10 PM: emergency-only.

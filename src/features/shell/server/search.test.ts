...
describe("search event entities", () => {
  ...
  it("finds a submission by bare numeric code, scoped to the event", async () => {
    ...
    expect(results).toHaveLength(1);
    expect(results[0]?.sublabel).toBe("SESS-42 · accepted");
  });
  ...
  it("finds a speaker by name and a session by title", async () => {
    ...
    expect(speaker).toEqual([{ type: "speaker", id: "c0000000-0000-4000-8000-000000000020", label: "Ada Lovelace", sublabel: "ada@example.com", href: `/events/${EVENT}/speakers/c0000000-0000-4000-8000-000000000020` }]);

    const session = await searchEventEntitiesIn(db, EVENT, "postgres at scale");
    expect(session).toEqual([{ type: "session", id: "c0000000-0000-4000-8000-000000000030", label: "Postgres at Scale", sublabel: "published", href: `/events/${EVENT}/agenda?view=list&session=c0000000-0000-4000-8000-000000000030` }]);
  });

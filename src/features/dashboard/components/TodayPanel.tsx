...
function dayKicker(timezone: string, days: number): string {
  const date = new Intl.DateTimeFormat("en-US", { timeZone: timezone, weekday: "long", month: "long", day: "numeric" }).format(new Date());
...
export function TodayPanel({ overview, firstName }: { overview: DashboardOverview; firstName: string }) {
  return <div className="dashboard-tab-panel">
    ...
    <div className="dashboard-today-grid">
      <WidgetBoundary name="forms"><FormProgressCards eventId={overview.event.id} timezone={overview.event.timezone} forms={overview.forms} /></WidgetBoundary>

...
export function DashboardPage() {
  ...
  const completion = Math.round((totalComplete / totalAssigned) * 100);
  ...
  return <div className="dashboard-page"><PageHeader eyebrow="SATURDAY, AUGUST 8" title="Good morning, Maya" description={`Here’s what needs your attention for ${event?.name ?? "your event"}.`} actions={<><Link className="button button-secondary" href={`/e/${event?.slug ?? "ai-engineer"}/schedule`} target="_blank">View live site <ArrowRight size={16} /></Link><Link className="button button-primary" href={`/events/${event?.id ?? ""}/forms`}>Create <span className="button-plus">＋</span></Link></>} />
    ...
    <section className="metric-grid">
      <article className="metric-card"><header><span className="metric-icon purple"><FileText size={20} /></span><span className="metric-trend up"><TrendingUp size={14} /> 18%</span></header><strong>247</strong><p>Total submissions</p><footer><span>12 new this week</span><Link href={`/events/${event?.id ?? ""}/abstracts`}>View all</Link></footer></article>
      ...
      <article className="metric-card"><header><span className="metric-icon amber"><CheckCircle2 size={20} /></span><span className="metric-label">Across 4 tasks</span></header><strong>{completion}%</strong><p>Onboarding complete</p><ProgressBar value={completion} tone="amber" /><footer><span>{totalAssigned - totalComplete} items remaining</span><Link href={`/events/${event?.id ?? ""}/tasks`}>Track</Link></footer></article>
      <article className="metric-card"><header><span className="metric-icon blue"><CalendarCheck size={20} /></span><span className="metric-label">2 days</span></header><strong>{scheduled.length + 24}</strong><p>Sessions scheduled</p><footer><span>4 still unscheduled</span><Link href={`/events/${event?.id ?? ""}/agenda`}>Open agenda</Link></footer></article>
    </section>

    <section className="dashboard-grid">
      <article className="panel submissions-chart-panel"><header className="panel-header"><div><h2>Submission momentum</h2><p>Applications received over the last eight weeks</p></div><select defaultValue="8"><option value="8">Last 8 weeks</option><option value="4">Last 4 weeks</option></select></header><div className="chart-summary"><strong>247</strong><span><TrendingUp size={14} /> +38 this week</span></div><div className="area-chart"><div className="chart-y"><span>60</span><span>40</span><span>20</span><span>0</span></div><div className="chart-plot"><i /><i /><i /><svg viewBox="0 0 700 180" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6958d7" stopOpacity=".25"/><stop offset="100%" stopColor="#6958d7" stopOpacity="0"/></linearGradient></defs><path className="area" d="M0,165 C60,152 70,140 120,143 C185,146 190,113 248,119 C305,125 320,86 375,91 C430,97 443,57 500,67 C550,75 590,38 700,20 L700,180 L0,180Z"/><path className="line" d="M0,165 C60,152 70,140 120,143 C185,146 190,113 248,119 C305,125 320,86 375,91 C430,97 443,57 500,67 C550,75 590,38 700,20"/><g>{[[0,165],[120,143],[248,119],[375,91],[500,67],[700,20]].map(([x,y]) => <circle key={x} cx={x} cy={y} r="4" />)}</g></svg><div className="chart-x"><span>Jun 15</span><span>Jun 29</span><span>Jul 13</span><span>Jul 27</span><span>Aug 8</span></div></div></div></article>

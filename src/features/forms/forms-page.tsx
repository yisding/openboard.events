...
export function FormsPage({ eventId }: { eventId: string }) {
  ...
  const eventForms = useMemo(() => state.forms.filter((form) => form.eventId === eventId), [state.forms, eventId]);
  const forms = useMemo(() => eventForms.filter((form) => tab === "all" || form.status === tab), [eventForms, tab]);

  function createForm() {
    if (!name.trim() || !event) return;
    const id = `${slugify(name)}-${Date.now().toString().slice(-5)}`;
    // A distinct participant section (id contains "speaker") so the builder's
    // Participant step and the public wizard split questions correctly.
    const form: FormRecord = {
      ...
      sections: [
        { id: `sec_session_${Date.now()}`, title: context === "cfp" ? "Your session" : "Your details", description: "Add the information our team needs.", fields: [
        ...
        { id: `sec_speaker_${Date.now()}`, title: "About you", description: "Tell us who is submitting.", fields: [

...
export function CfpWizard({ eventSlug, formId }: { eventSlug: string; formId: string }) {
  ...
  if (availability === "closed") return <main className="cfp-error"><Brand dark /><Clock3 size={36} /><h1>The call for speakers is closed.</h1><p>This form is not accepting new or updated submissions right now.</p><Link href={`/e/${event.slug}/schedule`}>View the event schedule</Link></main>;
...
function ParticipantStep({ values, setters, coSpeaker, setCoSpeaker, coName, setCoName, coEmail, setCoEmail, customFields, answers, onAnswer, errors, error }: { values: { firstName: string; lastName: string; company: string; jobTitle: string }; setters: { setFirstName: (v:string)=>void; setLastName:(v:string)=>void; setCompany:(v:string)=>void; setJobTitle:(v:string)=>void }; coSpeaker: boolean; setCoSpeaker:(v:boolean)=>void; coName: string; setCoName:(v:string)=>void; coEmail: string; setCoEmail:(v:string)=>void; customFields: FormFieldRecord[]; answers: Answers; onAnswer: (field: FormFieldRecord, value: AnswerValue) => void; errors: Record<string,string>; error: string | undefined }) {
  return <div className="cfp-step"><span className="cfp-step-count">STEP 4 OF 5</span><h1>Tell us about you</h1><p>This information becomes your speaker profile if the session is accepted.</p><div className="form-grid"><label><span>First name *</span><input value={values.firstName} onChange={(e)=>setters.setFirstName(e.target.value)} /></label><label><span>Last name *</span><input value={values.lastName} onChange={(e)=>setters.setLastName(e.target.value)} /></label><label><span>Company</span><input value={values.company} onChange={(e)=>setters.setCompany(e.target.value)} /></label><label><span>Job title</span><input value={values.jobTitle} onChange={(e)=>setters.setJobTitle(e.target.value)} /></label></div>{customFields.length > 0 && <div className="public-form-stack">{customFields.map((field)=><PublicField key={field.id} field={field} value={answers[field.key]} onChange={(value)=>onAnswer(field, value)} error={errors[field.key]} />)}</div>}{error && <div className="field-error">{error}</div>}<button className="add-cospeaker" onClick={()=>setCoSpeaker(!coSpeaker)}><UserPlus size={18}/><div><b>{coSpeaker ? "Co-speaker added" : "Add a co-speaker"}</b><small>You can invite another person to present with you.</small></div><span>{coSpeaker ? <Check size={16}/> : <ArrowRight size={16}/>}</span></button>{coSpeaker && <div className="co-speaker-fields form-grid"><label><span>Co-speaker name</span><input value={coName} onChange={(e)=>setCoName(e.target.value)} placeholder="Full name" /></label><label><span>Co-speaker email</span><input type="email" value={coEmail} onChange={(e)=>setCoEmail(e.target.value)} placeholder="speaker@company.com" /></label></div>}</div>;
}

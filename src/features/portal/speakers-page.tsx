...
export function SpeakersPage() {
  ...
  const speakers = useMemo(() => state.speakers.filter((speaker) => (confirmation === "all" || speaker.confirmation === confirmation) && `${speakerName(speaker)} ${speaker.company} ${speaker.email}`.toLowerCase().includes(search.toLowerCase())), [confirmation, search, state.speakers]);
  const active = state.speakers.find((item) => item.id === openId) ?? null;
  const outstanding = (speakerId: string) => state.tasks.filter((task) => !state.completions.some((done) => done.taskId === task.id && done.speakerId === speakerId)).length;
  return <>
    ...
    <section className="summary-row"><article><span className="summary-icon purple"><Users size={19} /></span><div><strong>{state.speakers.length + 70}</strong><small>Accepted speakers</small></div></article><article><span className="summary-icon green"><UserCheck size={19} /></span><div><strong>{state.speakers.filter((speaker) => speaker.confirmation === "confirmed").length + 68}</strong><small>Confirmed</small></div></article><article><span className="summary-icon amber"><span>!</span></span><div><strong>18</strong><small>Need attention</small></div></article><article><span className="summary-icon blue"><span>✓</span></span><div><strong>91%</strong><small>Average readiness</small></div></article></section>
    ...
    <SpeakerDrawer speaker={active} onClose={() => setOpenId(null)} onConfirmation={(value) => { if (active) dispatch({ type: "UPDATE_SPEAKER", speakerId: active.id, patch: { confirmation: value } }); toast("Confirmation updated"); }} />
  </>;
}

function SpeakerDrawer({ speaker, onClose, onConfirmation }: { speaker: SpeakerRecord | null; onClose: () => void; onConfirmation: (value: SpeakerRecord["confirmation"]) => void }) {
  const { state } = useDemo();
  ...
  return <Drawer open={Boolean(speaker)} onClose={onClose} title="Speaker profile"><div className="speaker-drawer"><div className="speaker-drawer-hero"><Avatar initials={speaker.avatar} color={speaker.avatarColor} size="xl" /><h2>{speakerName(speaker)}</h2><p>{speaker.title} at {speaker.company}</p><span>{speaker.location}</span><div><Link className="button button-primary" href="/portal/ai-engineer"><ExternalLink size={15} /> Open portal as {speaker.firstName}</Link><button className="button button-secondary"><Mail size={15} /> Email</button></div></div><div className="profile-overview"><div><span>Profile readiness</span><b>{speaker.profileCompletion}%</b></div><ProgressBar value={speaker.profileCompletion} /></div><section className="drawer-content"><h3>Confirmation</h3><div className="confirmation-options">{(["unconfirmed", "confirmed", "declined"] as const).map((value) => <button key={value} onClick={() => onConfirmation(value)} className={speaker.confirmation === value ? "active" : ""}>{value}</button>)}</div><h3>Biography</h3><p className="long-copy">{speaker.bio || "No biography submitted yet."}</p><h3>Sessions</h3>{submissions.map((submission) => <div className="mini-session" key={submission.id}><span>{submission.code}</span><b>{submission.title}</b><StatusBadge value={submission.status} /></div>)}<h3>Scheduled times</h3>{sessions.length ? sessions.map((session) => <div className="mini-session" key={session.id}><span>{session.startsAt ? new Date(session.startsAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }) : "Unscheduled"}</span><b>{session.title}</b><small>{session.room}</small></div>) : <p className="long-copy">No scheduled session yet.</p>}</section></div></Drawer>;
}

...
export function SpeakersPage({
  ...
  return <>
    ...
    <section className="summary-row"><article><span className="summary-icon accent"><Users size={19} /></span><div><strong>{eventSpeakers.length + 70}</strong><small>Accepted speakers</small></div></article><article><span className="summary-icon green"><UserCheck size={19} /></span><div><strong>{eventSpeakers.filter((speaker) => speaker.confirmation === "confirmed").length + 68}</strong><small>Confirmed</small></div></article><article><span className="summary-icon amber"><span>!</span></span><div><strong>18</strong><small>Need attention</small></div></article><article><span className="summary-icon blue"><span>✓</span></span><div><strong>91%</strong><small>Average readiness</small></div></article></section>
    ...
    <section className="panel data-panel"><div className="data-toolbar"><label className="table-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search speakers" aria-label="Search speakers" />{search && <button type="button" aria-label="Clear search" onClick={() => setSearch("")}><X size={14} /></button>}</label><button className="filter-button"><Filter size={15} /> Track <ChevronDown size={14} /></button><select className="compact-select" value={confirmation} onChange={(event) => setConfirmation(event.target.value)}><option value="all">All confirmations</option><option value="confirmed">Confirmed</option><option value="unconfirmed">Unconfirmed</option><option value="declined">Declined</option></select><span className="row-count">{speakers.length} shown</span></div>
      <div className="table-scroll"><table className="data-table speakers-table"><thead><tr><th><input type="checkbox" aria-label="Select all speakers" /></th><th>Speaker</th><th>Confirmation</th><th>Profile</th><th>Outstanding</th><th>Sessions</th><th /></tr></thead><tbody>{speakers.map((speaker) => {
        ...
        return <tr key={speaker.id} onClick={() => setOpenId(speaker.id)}><td onClick={(event) => event.stopPropagation()}><input type="checkbox" aria-label={`Select ${speakerName(speaker)}`} /></td><td><div className="speaker-table-person"><Avatar initials={speaker.avatar} color={speaker.avatarColor} /><div><b>{speakerName(speaker)}</b><span>{speaker.title} · {speaker.company}</span><small>{speaker.email}</small></div></div></td><td><StatusBadge value={speaker.confirmation} /></td><td><div className="profile-cell"><span>{speaker.profileCompletion}%</span><ProgressBar value={speaker.profileCompletion} tone={speaker.profileCompletion === 100 ? "green" : "accent"} /></div></td><td>{remaining ? <StatusBadge value={`${remaining} tasks`} /> : <StatusBadge value="Ready" />}</td><td><span className="session-count">{sessions.length}</span></td><td><button type="button" className="icon-button" aria-label={`Actions for ${speakerName(speaker)}`}><MoreHorizontal size={17} /></button></td></tr>;

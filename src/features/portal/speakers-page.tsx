...
import { Avatar, Button, Drawer, PageHeader, ProgressBar, Select, StatusBadge } from "@/shared/ui/ui-kit";
  ...
  return <>
    ...
    <section className="panel data-panel"><div className="data-toolbar"><label className="table-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search speakers" aria-label="Search speakers" />{search && <button type="button" aria-label="Clear search" onClick={() => setSearch("")}><X size={14} /></button>}</label><Select className="compact-select" aria-label="Filter by confirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)}><option value="all">All confirmations</option><option value="confirmed">Confirmed</option><option value="unconfirmed">Unconfirmed</option><option value="declined">Declined</option></Select><span className="row-count">{speakers.length} shown</span></div>
      <div className="table-scroll"><table className="data-table speakers-table"><thead><tr><th>Speaker</th><th>Confirmation</th><th>Profile</th><th>Outstanding</th><th>Sessions</th><th /></tr></thead><tbody>{speakers.map((speaker) => {
        ...
        return <tr key={speaker.id} onClick={() => setOpenId(speaker.id)}><td><div className="speaker-table-person"><Avatar initials={speaker.avatar} color={speaker.avatarColor} /><div><b>{speakerName(speaker)}</b><span>{speaker.title} · {speaker.company}</span><small>{speaker.email}</small></div></div></td><td><StatusBadge value={speaker.confirmation} /></td><td><div className="profile-cell"><span>{speaker.profileCompletion}%</span><ProgressBar label={`Profile completion for ${speakerName(speaker)}`} value={speaker.profileCompletion} tone={speaker.profileCompletion === 100 ? "green" : "accent"} /></div></td><td>{remaining ? <StatusBadge value={`${remaining} tasks`} /> : <StatusBadge value="Ready" />}</td><td><span className="session-count">{sessions.length}</span></td><td><button type="button" className="icon-button" aria-label={"Open " + speakerName(speaker)} onClick={(event)=>{event.stopPropagation();setOpenId(speaker.id)}}><MoreHorizontal size={17} /></button></td></tr>;
      })}</tbody></table></div><div className="table-footer"><span>Showing {speakers.length} speakers</span></div>
    </section>
    <SpeakerDrawer speaker={active} eventTimezone={event?.timezone ?? "UTC"} onClose={() => setOpenId(null)} onOpenPortal={openPortalAs} onConfirmation={(value) => { if (active) dispatch({ type: "UPDATE_SPEAKER", speakerId: active.id, patch: { confirmation: value } }); toast("Confirmation updated"); }} />
  </>;
}

export function SpeakerDrawer({ speaker, eventTimezone, onClose, onOpenPortal, onConfirmation }: { speaker: SpeakerRecord | null; eventTimezone: string; onClose: () => void; onOpenPortal: (speaker: SpeakerRecord) => void; onConfirmation: (value: SpeakerRecord["confirmation"]) => void }) {
  const { state } = useDemo();
  ...
  return <Drawer open={Boolean(speaker)} onClose={onClose} title="Speaker profile"><div className="speaker-drawer"><div className="speaker-drawer-hero"><Avatar initials={speaker.avatar} color={speaker.avatarColor} size="xl" /><h2>{speakerName(speaker)}</h2><p>{speaker.title} at {speaker.company}</p><span>{speaker.location}</span><div><Button onClick={() => onOpenPortal(speaker)}><ExternalLink size={15} /> Open portal as {speaker.firstName}</Button><a className="button button-secondary" href={"mailto:" + speaker.email}><Mail size={15} /> Email</a></div></div><div className="profile-overview"><div><span>Profile readiness</span><b>{speaker.profileCompletion}%</b></div><ProgressBar label={`Profile completion for ${speakerName(speaker)}`} value={speaker.profileCompletion} /></div><section className="drawer-content"><h3>Confirmation</h3><div className="confirmation-options">{(["unconfirmed", "confirmed", "declined"] as const).map((value) => <button key={value} type="button" onClick={() => onConfirmation(value)} className={speaker.confirmation === value ? "active" : ""}>{value}</button>)}</div><h3>Biography</h3><p className="long-copy">{speaker.bio || "No biography submitted yet."}</p><h3>Sessions</h3>{submissions.map((submission) => <div className="mini-session" key={submission.id}><span>{submission.code}</span><b>{submission.title}</b><StatusBadge value={submission.status} /></div>)}<h3>Scheduled times</h3>{sessions.length ? sessions.map((session) => <div className="mini-session" key={session.id}><span>{session.startsAt ? <TzTime instant={session.startsAt} tz={eventTimezone} style={{ month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }} /> : "Unscheduled"}</span><b>{session.title}</b><small>{session.room}</small></div>) : <p className="long-copy">No scheduled session yet.</p>}</section></div></Drawer>;
}

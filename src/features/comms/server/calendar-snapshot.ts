...
export function parseCalendarEventSnapshot(value: unknown): CalendarEventSnapshot {
  ...
  if (!parsed.success) throw new AppError("VALIDATION", "stored calendar event snapshot is invalid");
  return parsed.data;
}
...
export function parseCalendarCancellationSnapshot(value: unknown): CalendarCancellationSnapshot {
  ...
  if (!parsed.success) throw new AppError("VALIDATION", "stored calendar cancellation snapshot is invalid");
  return parsed.data;
}

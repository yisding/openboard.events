...
export type EmailLayoutMeta = {
  ...
  /** CAN-SPAM's required physical postal address (P3-EMAIL). Already escaped by the caller, same as `eventName`/`unsubscribeUrl`. */
...
export function emailLayout(bodyHtml: string, key: TemplateKey, meta: EmailLayoutMeta): string {
  ...
  // Every non-essential send carries a working one-click unsubscribe link —
  // not just `task_reminder` — per the fleet-wide policy in
  // `isTransactionalTemplate`; decision/schedule/portal-login mail never did
  // and still doesn't.

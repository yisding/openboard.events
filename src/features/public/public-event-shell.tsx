...
export function PublicEventShell({ children, active, event, embed = false, embedOptions = DEFAULT_EMBED_OPTIONS }: { children: React.ReactNode; active: "schedule" | "speakers"; event: EventRecord; embed?: boolean; embedOptions?: EmbedOptions }) {
  if (embed) return <div className={`embed-shell ${embedOptions.theme === "dark" ? "embed-dark" : ""}`} style={{ "--purple": embedOptions.accent } as React.CSSProperties}><EmbedResizer />{embedOptions.header && <header className="embed-header"><b>{event.name}</b><span>Sep 15–16, 2026 · {event.city}</span></header>}{children}<footer>Powered by <b>openboard</b></footer></div>;
  return <div className="public-event">
    <header className="public-event-header"><div className="public-event-container"><span className="public-event-logo">AI<span>.engineer</span></span><nav><Link className={active === "schedule" ? "active" : ""} href={`/e/${event.slug}/schedule`}>Schedule</Link><Link className={active === "speakers" ? "active" : ""} href={`/e/${event.slug}/speakers`}>Speakers</Link><Link href={`/submit/${event.slug}/technical-talks`}>Call for speakers</Link></nav><Link className="button public-cta" href={`/portal/${event.slug}`}>Speaker portal</Link></div></header>

...
import { listReviewHistory } from "@/features/submissions";
import { eventIdSchema, submissionIdSchema } from "@/shared/contracts";
import { defineHandler } from "@/shared/server/handler";

export const dynamic = "force-dynamic";

/** Attributed score history is governance data, so reviewers never receive it. */
const history = defineHandler({
  auth: adminAuth({ role: "organizer" }),
  input: z.object({ submissionId: submissionIdSchema }),
  handler: async ({ eventId, input }) => ({
    entries: await listReviewHistory(eventIdSchema.parse(eventId), input.submissionId),
  }),
});

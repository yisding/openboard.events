...
import { createTaskIn, saveTaskInputSchema } from "@/features/portal/tasks-admin/server/mutations";
...
const create = defineHandler({
  ...
  handler: async ({ eventId, input }) => createTaskIn(db, eventIdSchema.parse(eventId), input),
});

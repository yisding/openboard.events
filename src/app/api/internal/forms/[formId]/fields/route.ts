...
import { COMMITTED_FIELD_TYPES, eventIdSchema, formIdSchema, mapsToTargetSchema, sectionIdSchema } from "@/shared/contracts";
...
const create = defineHandler({
  ...
  handler: async ({ eventId, input, params }) => withTx((tx) => createFieldIn(tx, eventIdSchema.parse(eventId), paramsSchema.parse(params).formId, input, input.expectedUpdatedAt)),
});

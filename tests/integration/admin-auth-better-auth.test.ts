...
describe("M42 admin auth on Better Auth", () => {
  ...
  beforeAll(async () => {
    ...
    // but the backfilled credential row retains its migration-era created_at.
    ...
    // Conversely, a newer unverified signup can also carry a modern hash. Its
    // credential postdates the 0010 marker and must not be grandfathered.
    const newerHash = await hashAdminPassword(MODERN_PASSWORD);
    await pglite.query(
      "INSERT INTO users(id,email,name,password_hash,created_at) SELECT $1,'newer-unverified@example.com','Newer Unverified',$2,created_at + interval '1 second' FROM organizations WHERE id='d3fa0000-0000-4000-8000-000000000001'",
      [newerUnverifiedUser, newerHash],
    );
    await pglite.query(
      "INSERT INTO admin_accounts(user_id,account_id,provider_id,password,created_at,updated_at) SELECT $1::uuid,$1::uuid::text,'credential',$2,created_at + interval '1 second',created_at + interval '1 second' FROM organizations WHERE id='d3fa0000-0000-4000-8000-000000000001'",
      [newerUnverifiedUser, newerHash],
    );
  ...
  it("keeps the fallback provider usable for established legacy credentials", async () => {
    const verification = await database.select({ id: users.id, emailVerified: users.emailVerified })
      .from(users)
      .where(inArray(users.id, [legacyUser, resetLegacyUser, newerUnverifiedUser]));

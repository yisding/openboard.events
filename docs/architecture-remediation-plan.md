...
The first remediation is included in the PR that introduced this plan:

- [x] Retire fallback admin authentication. Better Auth is now the only admin
  identity and credential provider; speaker portal OTP/magic-link auth remains
  separate. The provider switch, stateless admin JWT, password mirroring, and
  test-only cookie-minting endpoint are removed. Migration 0033 erases and
  constrains the retired `users.password_hash` store, and browser automation
  signs in through the real password endpoint.
...
1. Fallback-auth retirement and this plan (current PR).

...
| 39 | After publishing, request the public schedule until regeneration settles | A fresh response has `s-maxage`, or a cached response reports `x-nextjs-cache: HIT`; `STALE` alone is not a pass, its `data-openboard-build` marker matches `/api/health`, and the newly published session is in the payload |
...
| 16 | *(C)* Request a public page and an embed until regeneration settles | A fresh response has `s-maxage`, or a cached response reports `x-nextjs-cache: HIT`; `STALE` alone is not a pass, and each body marker matches `/api/health`'s build SHA |

...
3. **Scheduled-dispatch status** — `workers/jobs/dispatch.ts` gives every web request a 120-second
...
| `jobs.outboxLastSuccessAgeSeconds` | `0`–`180` | `> 180` | missing/null or `> 300` | Outbox completes every minute even with no queued mail. Staleness therefore catches a stopped Cron, broken Service Binding/entrypoint, compatibility-auth failure, or broken job route when queue metrics remain empty. |
